...
DB에는 UTC를 유지시키고 싶고, 프론트에선 로컬타임으로 보여주기 위해 그냥 Serializer에서의 이 코드를 제거했다.
...
이후 프론트에서 찍히는 데이터 시간을 보니 한국 로컬타임으로 그냥 잘 보내주고 있었다. 굳이 백엔드에서 SerializerMethodField를 사용할 이유가 없었던 것.
